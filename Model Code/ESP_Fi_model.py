...
class ESP_Fi_ResNet(nn.Module):
    ...
    def _make_layer(self, block, out_channels, blocks, stride=(1, 1)):
        ...
        layers.append(block(self.in_channels, out_channels,
                      stride=stride, i_downsample=i_downsample))
    ...
    def forward(self, x):
        ...
        if self.pos_embed is None or self.pos_embed.shape[1] != seq_len:
            self.pos_embed = nn.Parameter(torch.randn(
                1, seq_len, self.emb_size)).to(device)
...
class MobileNetV3Block(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, exp_size, se=True, nl='HS'):
        ...
        self.conv1 = nn.Conv2d(in_channels, exp_size,
        ...
        self.conv3 = nn.Conv2d(exp_size, out_channels,
                               kernel_size=1, padding=0, bias=False)
...
class MobileNetV3(nn.Module):
    def __init__(self, num_classes=7, input_channels=1):
        ...
        self.stem = nn.Sequential(
            nn.Conv2d(input_channels, 16, kernel_size=(3, 3),
        ...
        self.blocks = nn.Sequential(
            MobileNetV3Block(16, 16, kernel_size=(3, 3), stride=(
                1, 1), exp_size=16, se=False, nl='RE'),
            MobileNetV3Block(16, 24, kernel_size=(3, 3), stride=(
                2, 2), exp_size=72, se=False, nl='RE'),
            MobileNetV3Block(24, 24, kernel_size=(3, 3), stride=(
                1, 1), exp_size=88, se=False, nl='RE'),
            MobileNetV3Block(24, 40, kernel_size=(5, 5), stride=(
                2, 2), exp_size=96, se=True, nl='HS'),
            MobileNetV3Block(40, 40, kernel_size=(5, 5), stride=(
                1, 1), exp_size=240, se=True, nl='HS'),
            MobileNetV3Block(40, 40, kernel_size=(5, 5), stride=(
                1, 1), exp_size=240, se=True, nl='HS'),
            MobileNetV3Block(40, 48, kernel_size=(5, 5), stride=(
                1, 1), exp_size=120, se=True, nl='HS'),
            MobileNetV3Block(48, 48, kernel_size=(5, 5), stride=(
                1, 1), exp_size=144, se=True, nl='HS'),
            MobileNetV3Block(48, 96, kernel_size=(5, 5), stride=(
                2, 2), exp_size=288, se=True, nl='HS'),
            MobileNetV3Block(96, 96, kernel_size=(5, 5), stride=(
                1, 1), exp_size=576, se=True, nl='HS'),
            MobileNetV3Block(96, 96, kernel_size=(5, 5), stride=(
                1, 1), exp_size=576, se=True, nl='HS'),
        )
...
class InvertedResidualBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, expand_ratio, se=True):
        ...
        self.conv1 = nn.Conv2d(in_channels, expand_channels,
                               kernel_size=1, padding=0, bias=False)
        ...
        self.conv3 = nn.Conv2d(expand_channels, out_channels,
                               kernel_size=1, padding=0, bias=False)
        self.bn3 = nn.BatchNorm2d(out_channels)
...
class EfficientNetLite(nn.Module):
    def __init__(self, num_classes=7, input_channels=1):
        ...
        self.blocks = nn.Sequential(
            ...
            InvertedResidualBlock(32, 16, kernel_size=3,
                                  stride=1, expand_ratio=1, se=False),
            InvertedResidualBlock(16, 24, kernel_size=3,
                                  stride=2, expand_ratio=6, se=False),
            InvertedResidualBlock(24, 24, kernel_size=3,
                                  stride=1, expand_ratio=6, se=False),


            InvertedResidualBlock(24, 40, kernel_size=5,
                                  stride=2, expand_ratio=6, se=True),
            InvertedResidualBlock(40, 40, kernel_size=5,
                                  stride=1, expand_ratio=6, se=True),


            InvertedResidualBlock(40, 80, kernel_size=3,
                                  stride=2, expand_ratio=6, se=True),
            InvertedResidualBlock(80, 80, kernel_size=3,
                                  stride=1, expand_ratio=6, se=True),
            InvertedResidualBlock(80, 112, kernel_size=5,
                                  stride=1, expand_ratio=6, se=True),
            InvertedResidualBlock(112, 112, kernel_size=5,
                                  stride=1, expand_ratio=6, se=True),
            InvertedResidualBlock(112, 192, kernel_size=5,
                                  stride=2, expand_ratio=6, se=True),
            InvertedResidualBlock(192, 192, kernel_size=5,
                                  stride=1, expand_ratio=6, se=True),
            InvertedResidualBlock(192, 192, kernel_size=5,
                                  stride=1, expand_ratio=6, se=True),
            InvertedResidualBlock(192, 320, kernel_size=3,
                                  stride=1, expand_ratio=6, se=True)
        )
