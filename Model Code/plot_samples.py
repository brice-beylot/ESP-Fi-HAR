...
mat_files = glob.glob(
    "C:/Users/brice/Desktop/Brice/Centrale Supelec/1ère année/Stage Zoe Care/ESP-Fi-HAR-1.0/Model Code/Data/EnvironmentNo.1(corridor)/mat/1-1-7-1.mat")
...
for mat_path in mat_files:
    try:
        ...
        x_filtered = filtfilt(b, a, x, axis=0)
        ...
        ax1.plot(x[0, :, :], alpha=0.6)
        ax1.set_title(f"1. ORIGINAL DATA: {filename}", fontweight='bold')
        ax1.set_ylabel("Amplitude")
        ax1.grid(True, alpha=0.3)

        # 2. First Derivative (Velocity)
        ax2 = fig.add_subplot(2, 1, 2)
        velocity = np.gradient(x[0, :, :], axis=0)
        ax2.plot(velocity, alpha=0.6)
        ax2.set_title("2. FIRST DERIVATIVE (VELOCITY)", fontweight='bold')
